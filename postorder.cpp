...
public:
  ...
  string visit()
  {
    /*Algoritmo di visita dell'albero in post-ordine.
      ...
      la visita semplicemente ricostruisce la rappresentazione esterna (come stringa)
...
AST *create(string linearrep, set<char> alphabet, bool verbose)
{
  ...
  if (n == 1 or n == 3)
  {                                       // linearrep = 'c' oppure "(c)"
    char c = linearrep.at(max(0, n - 2)); // Unifica i due casi recuperando la lettera
    if (alphabet.count(c) == 0)
      throw invalid_argument("Il simbolo " + string(1, c) + " non fa parte dell'alfabeto");
    return new AST(c);
  }
  char op = linearrep.at(1);
  if (operators.count(op) == 0)
    throw invalid_argument("Il simbolo " + string(1, op) + " non è un operatore legale");
  int start = 2;
  int stop = getsubtree(linearrep, start);
  string left = linearrep.substr(start, stop);
  start += stop;
  if (op == '*')
  { // L'albero andrà costruito con un solo figlio
    if (verbose)
      cout << "op:" << op << "\tleft:" << left << endl;
    AST *lt = create(left, alphabet, verbose);
    return new AST(op, lt, nullptr);
  }
  else
  { // Operatore binario -> due figli
...
int main(int argc, char **argv)
{
  ...
  char symb;
  while (alphabet >> symb)
  { // Se il primo parametro è un oggetto di tipo
